...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold border ${s.bg} ${s.border} ${s.text}`}>
      {s.icon}{status}
    </span>
  );
}
      ...
      <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        <div className="px-6 py-5 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white text-sm">ยืนยันการลบออเดอร์</h3>
          ...
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 text-xs text-amber-400 space-y-1">
            ...
            <p className="text-amber-400/90">ออเดอร์จะถูกซ่อนในหน้า Admin เท่านั้น ข้อมูลใน Google Sheets จะยังคงอยู่ครบ</p>
          </div>
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 py-2.5 text-sm font-semibold text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors">
              ยกเลิก
            </button>
            <button onClick={onConfirm} className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white bg-red-600 hover:bg-red-500 rounded-xl transition-colors">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
      <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <div>
            <p className="text-xs text-slate-500 font-semibold">อัปเดตสถานะ</p>
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-2">สถานะ</label>
            <div className="relative">
              <select
                value={status}
                onChange={(e) => setStatus(e.target.value)}
                className="w-full appearance-none bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none focus:border-indigo-500 transition-colors pr-10"
              >
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-2">Tracking Number <span className="text-slate-600 font-normal">(ถ้ามี)</span></label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 text-white placeholder-slate-600 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-colors"
              />
            </div>
          </div>
          {error && (
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 py-2.5 text-sm font-semibold text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors">ยกเลิก</button>
            <button onClick={handleSave} disabled={saving} className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 rounded-xl transition-colors">
...
function OrderCard({
  ...
  return (
    <div className={`rounded-2xl border transition-colors ${isDeleted
      ...
      >
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
            ...
            <span className="text-emerald-400 font-semibold">{order.TotalPrice.toLocaleString()} บ.</span>
          ...
          {!isDeleted && (
            <>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-300 bg-indigo-600/10 hover:bg-indigo-600/20 border border-indigo-500/20 rounded-lg transition-colors"
              >
      ...
      <div className={`overflow-hidden transition-all duration-300 ${expanded ? 'max-h-[2000px]' : 'max-h-0'}`}>
        <div className="px-4 pb-5 border-t border-slate-800 pt-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-slate-800/40 rounded-xl p-4 space-y-3">
              <h4 className="text-xs font-semibold text-slate-400">รายละเอียดการจัดส่ง</h4>
              <div className="space-y-2 text-xs">
                <div className="flex gap-2">
                  <User className="w-3.5 h-3.5 text-slate-500 mt-0.5 shrink-0" />
                  <div><span className="text-slate-500">ชื่อผู้รับ: </span><span className="text-slate-200 font-medium">{order.CustomerName}</span></div>
                </div>
                <div className="flex gap-2">
                ...
                {order.TrackingNumber && (
                  <div className="flex gap-2">
                    ...
                    <div><span className="text-slate-500">Tracking: </span>
                      <span className="text-emerald-400 font-semibold font-mono">{order.TrackingNumber}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="bg-slate-800/40 rounded-xl p-4 space-y-3">
              <h4 className="text-xs font-semibold text-slate-400">การชำระเงิน</h4>
              <div className="space-y-2 text-xs">
                <div className="flex gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <div><span className="text-slate-500">ยอดรวม: </span>
                    <span className="text-emerald-400 font-semibold text-sm">{order.TotalPrice.toLocaleString()} บาท</span>
                    <span className="text-slate-500 ml-1">({order.TotalItems} ตัว)</span>
                  </div>
                </div>
                {order.PromoCode && (
                  <div className="flex gap-2 items-center">
                    <span className="inline-flex items-center gap-1 text-[10px] text-indigo-300 bg-indigo-600/10 border border-indigo-500/20 px-2 py-0.5 rounded-lg font-semibold">
                      โค้ดส่วนลด: {order.PromoCode}
                    </span>
                  </div>
                )}
                <div className="flex gap-2 items-start">
                  ...
                  <div>
                    ...
                    {isSlipValid ? (
                      ...
                      <span className="text-red-400 font-medium">ไม่มีสลิป / อัปโหลดไม่สำเร็จ</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div>
            <h4 className="text-xs font-semibold text-slate-400 mb-2.5">รายการเสื้อ ({items.length} ตัว)</h4>
            <div className="rounded-xl border border-slate-800 overflow-hidden">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-800/60 text-slate-500 font-semibold">
                    <th className="px-3 py-2 text-center w-10">#</th>
                    <th className="px-3 py-2 text-left">แบบ</th>
                    <th className="px-3 py-2 text-center w-12">ไซส์</th>
                    <th className="px-3 py-2 text-left">ชื่อสกรีน</th>
                    <th className="px-3 py-2 text-center w-12">เบอร์</th>
                    <th className="px-3 py-2 text-left">หมายเหตุ</th>
                    <th className="px-3 py-2 text-right w-20">ราคา</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/60">
                  {items.map((item) => (
                    <tr key={item.ItemIndex} className="text-slate-300 hover:bg-slate-800/20 transition-colors">
                      <td className="px-3 py-2.5 text-center text-slate-500 font-medium">{item.ItemIndex}</td>
                      <td className="px-3 py-2.5 font-medium text-slate-100">{item.DesignName}</td>
                      <td className="px-3 py-2.5 text-center font-semibold">{item.Size}</td>
                      <td className="px-3 py-2.5">{item.PrintName || <span className="text-slate-600">-</span>}</td>
                      <td className="px-3 py-2.5 text-center font-mono font-medium">{item.BackNumber || <span className="text-slate-600">-</span>}</td>
                      <td className="px-3 py-2.5 text-slate-400">{item.CustomText || <span className="text-slate-600">-</span>}</td>
                      <td className="px-3 py-2.5 text-right font-semibold text-slate-100">{item.ItemPrice} บ.</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-slate-800/40 border-t border-slate-700">
                    <td colSpan={6} className="px-3 py-2.5 text-right text-xs font-semibold text-slate-400">รวมทั้งหมด</td>
                    <td className="px-3 py-2.5 text-right text-sm font-bold text-emerald-400">{order.TotalPrice.toLocaleString()} บ.</td>
                  </tr>
                </tfoot>
              </table>
            </div>
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      {/* Navbar */}
      <header className="sticky top-0 z-30 bg-slate-950/95 backdrop-blur-sm border-b border-slate-800 px-4 md:px-6 py-4 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center">
            <Package className="w-4 h-4 text-white" />
          </div>
          <div>
            <h1 className="text-sm font-bold text-white">Buucuties.jersey Admin</h1>
            <p className="text-[10px] text-slate-500">จัดการคำสั่งซื้อ</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => fetchOrders(true)} disabled={refreshing}
            className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors" title="รีเฟรช">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-5">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {[
            ...
            <div key={stat.label} className={`rounded-xl border p-3 space-y-0.5 ${stat.bg}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 text-slate-100 placeholder-slate-600 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-colors"
            />
            {search && (
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-semibold transition-colors ${!showDeleted ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'}`}
            >
              ออเดอร์ทั่วไป
            </button>
            <button
              onClick={() => { setShowDeleted(true); setFilterStatus('ทั้งหมด'); }}
              className={`px-4 py-2.5 rounded-xl text-xs font-semibold transition-colors ${showDeleted ? 'bg-red-600 text-white' : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'}`}
            >
          ...
          <div className="flex gap-2 flex-wrap">
            {['ทั้งหมด', ...STATUS_OPTIONS].map((s) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors whitespace-nowrap ${filterStatus === s
